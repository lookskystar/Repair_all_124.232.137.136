...
<script type="text/javascript">
	...
	function submitForm(){
		...
		$.post("<%=basePath%>checkDealJc!roleSign.do",{"uname":uname,"pwd":pwd,"cardnum":cardnum,"dpId":dpId},function(data){
			if(data=="failure"){
				...
				if(type=="1"||type==1){//交车工长
					top.jcmain.jcgz_gdt.document.getElementById("gzgh").value=datas[1];
				    top.jcmain.jcgz_gdt.document.getElementById("gzxm").value=datas[2];
				}else if(type=="2"||type==2){//检修主任
					top.jcmain.jcgz_gdt.document.getElementById("zrgh").value=datas[1];
				    top.jcmain.jcgz_gdt.document.getElementById("zrxm").value=datas[2];
				}else if(type=="3"||type==3){//验收员
					top.jcmain.jcgz_gdt.document.getElementById("ysgh").value=datas[1];
				    top.jcmain.jcgz_gdt.document.getElementById("ysxm").value=datas[2];
				}else if(type=="4"||type==4){//段长jcgz_gdt
					top.jcmain.jcgz_gdt.document.getElementById("dzgh").value=datas[1];
				   	top.jcmain.jcgz_gdt.document.getElementById("dzxm").value=datas[2];
				}
				$("#cardnum").val("");
				$("#username").val("");
				$("#password").val("");
