...
<script type="text/javascript">
	...
	$(function(){
         ...
         <c:forEach items="${units}" var="unit" varStatus="index">
         	d.add(${index.index+1},0,'${unit.firstunitname }','<%=basePath%>query!view.do?jcStype=${jcStype }&jcNum=${jcNum }&xcxc=${xcxc }&fristUnit=${unit.firstunitid }&rjhmId=${datePlan.rjhmId }');
 		 </c:forEach>
 		d.add('t_0',-1,'检修班组');//班组树形
 		<c:forEach items="${bzs}" var="bz" varStatus="index">
         	d.add('t_${index.index+1}','t_0','${bz.proteamname }','<%=basePath%>query!getInfoByBZ.do?jcStype=${jcStype }&jcNum=${jcNum }&xcxc=${xcxc }&teamId=${bz.proteamid }&rjhmId=${datePlan.rjhmId }','${bz.proteamname }','_blank');
 		</c:forEach>
...
<tr><td colspan="6">
<table width="864" border="0" align="center" cellspacing="0" vspace="0" style="font-size:10pt;">
 <tr>
