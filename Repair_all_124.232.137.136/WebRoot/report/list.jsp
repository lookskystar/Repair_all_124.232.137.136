		...
		<c:forEach var="dp" items="${list }">
			<tr>
				<td style="text-align: center;">
				  <c:if test="${dp.projectType==0}">${dp.jcType }</c:if>
				  <c:if test="${dp.projectType==1}"><a href="<%=basePath%>query!findJCAll.do?rjhmId=${dp.rjhmId}&jcStype=${dp.jcType }" target="_blank">${dp.jcType }</a></c:if>
				</td>
				<td style="text-align: center;">
					${dp.jcnum }
				</td>
				<td style="text-align: center;">${dp.fixFreque }</td>
				<td style="text-align: center;">${dp.gdh }</td>
				<td style="text-align: center;">${dp.twh }</td>
				<td style="text-align: center;">${dp.kcsj }</td>
				<td style="text-align: center;">${fn:substring(dp.jhqjsj,5,16) }--${fn:substring(dp.sjqjsj,5,16) }</td>
				<td style="text-align: center;">${fn:substring(dp.jhjcsj,5,16) }--${fn:substring(dp.sjjcsj,5,16) }</td>
				...
				<td style="text-align: center;">
					<c:if test="${dp.projectType==0}">
						<a href="<%=basePath%>query!view.do?rjhmId=${dp.rjhmId }" target="_blank">记录查看</a>
					</c:if>
					<c:if test="${dp.projectType==1}">
						<a href="<%=basePath%>query!zxView.do?rjhmId=${dp.rjhmId}" target="_blank">记录查看</a>
					</c:if>
				</td>
			</tr>
		</c:forEach>
		<c:if test="${empty list}"><td colspan="12" style="text-align: center;">该时间段内无相应机车信息!</td></c:if>
		<tr></tr>
	</table>
</body>
</html>