...
<script language="JavaScript" type="text/javascript">
    ...
    function SaveAsFile(){ 
		...
		LODOP.SAVE_TO_FILE("${datePlan.jcType }-${datePlan.jcnum }-${datePlan.fixFreque}整车检修配件记录.xls"); 
	};	
	...
	function CreatePrintPage(){
		...
		LODOP.ADD_PRINT_TEXT(21,300,300,30,"${datePlan.jcType }-${datePlan.jcnum }整车检修配件记录\n");
...
<ul id="nav">
    <li><a href="javascript:void(0);" style="background-color:#fff;color:blue;">◇整车记录 </a>
       <ul>
         ...
         <li><a href="<%=basePath%>query!findJCAll.do?rjhmId=${datePlan.rjhmId}&jcStype=${datePlan.jcType }" style="background-color:#fff;color:blue;">检修配件记录</a></li>
       </ul>
    </li>
 ...
 <tr>
	...
	<table width="864" border="0" align="center" cellspacing="0" vspace="0" id="datatabel">
		<tr style="line-height:40px;height: 40px;background-color: #328aa4;font-weight: bolder;">
			<td class="tbCELL1" align="center" style="white-space:nowrap;color:#ffffff;" width="10%">班组</td>
			<td class="tbCELL1" align="center" style="white-space:nowrap;color:#ffffff;" width="10%">序号</td>
			<td class="tbCELL1" align="center" style="white-space:nowrap;color:#ffffff;" width="30%">记录名称</td>
		</tr>
 		<c:forEach var="map" items="${map}">
 		  <c:set var="num" value="0"/>
 		  <c:if test="${!empty map.value}"><c:set var="num" value="${fn:length(map.value)}"/></c:if>
 		  <tr>
 		  	<td rowspan="${num+1}" class="tbCELL1" align="center" style="white-space:nowrap" width="10%">${map.key.proteamname}(${num+1})</td>
 		  	<td class="tbCELL1" align="center" style="white-space:nowrap" width="10%">1</td>
 		  	<td class="tbCELL1" align="center" style="white-space:nowrap" width="50%"><a href="query!getZxInfoByBZ.do?workFlag=1&rjhmId=${datePlan.rjhmId}&teamId=${map.key.proteamid}" target="_blank">${map.key.proteamname}中修检修记录</a></td>
 		  </tr>
 		  <c:if test="${!empty map.value}">
 		    <c:forEach var="pj" items="${map.value}" varStatus="s">
 		      <tr><td class="tbCELL1" align="center" style="white-space:nowrap" width="10%">${s.index+2}</td>
 		      <td class="tbCELL1" align="center" style="white-space:nowrap" width="30%"><a href="<%=basePath %>query!zxFittingDetailView.do?id=${datePlan.rjhmId }&staticId=${pj.pjsid}" target="_blank">${pj.pjName}</a></td>
 		      </tr>
