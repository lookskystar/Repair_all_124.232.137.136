...
<script type="text/javascript">
   ...
   function getFirstUnit(){
    ...
    function addItem(){
    	...
    	diag.Title = "项目添加";
    	diag.URL = "<%=basePath%>fixItemAction!addItemInput.do";
    	diag.Width=650;
    	diag.Height=600;
    	diag.show();
    }
    ...
    function editItemInfo(itemId){
    	top.Dialog.open({URL:"<%=basePath%>fixItemAction!editItemInput.do?itemId="+itemId,Width:650,Height:600,Title:"项目修改"});
    }

    //删除项目
    function deleteItemInfo(itemId){
    	top.Dialog.confirm("确认删除吗？",function(){
    		$.post("fixItemAction!deleteItemInfo.do",{"itemId":itemId},function(result){
            	if(result=="success"){
    ...
    function deleteItem(){
    	...
    	$.ajax({
			type:"post",
			async:false,
			url:"fixItemAction!deleteItemInfo.do",
			data:{"itemId":items.join(",")},
...
<body>
	<form id="form" action="fixItemAction!itemListInput.do" method="post">
	<div class="box2">
		<table>
			<tr>
				<td>
	            ...
	            <td>
	            	大&nbsp;&nbsp;部&nbsp;&nbsp;件：
	                <select id="first" onchange="getSecondUnit();" class="default" name="first">
	        ...
	        <tr>
	            <td>
	            	二级部件：
	                <select id="second" class="default" name="second">
	...
	<div>
		<table width="100%" class="tableStyle" useCheckBox="true">
			<tr>
				<th width="30px" align="center">b</th>
		      	<th width="10%" align="center"><span>大部件名称</span></th>
		      	<th width="10%" align="center"><span>二级部件名称</span></th>
		      	<th width="10%" align="center"><span>部位名称</span></th> 
		      	<th width="20%" align="center"><span>项目名称</span></th>
		      	<th width="10%" align="center"><span>检修时长</span></th>
			...
			<c:if test="${!empty pm.datas}">
				<c:forEach items="${pm.datas}" var="item">
				   <tr>
				    <td width="30px" align="center"><input name="item" value="${item.thirdUnitId}" type="checkbox" id="${item.thirdUnitId}"/></td>
				    <td width="10%" align="center">${item.unitName}</td>
					<td width="10%" align="center">
						<span title="${item.secUnitName}">
							${fn:substring(item.secUnitName,0,5)}...
						</span>
					</td>
					...
					<td width="10%" align="center">${item.banzuId.proteamname}</td>
					<td width="20%" align="center">
					     <a href="javascript:;" style="color:blue;" onclick="getItemInfo(${item.thirdUnitId});">查看</a>&nbsp;&nbsp;
					     <a href="javascript:;" style="color:blue;" onclick="editItemInfo(${item.thirdUnitId});">修改</a>&nbsp;&nbsp;
					     <a href="javascript:;" style="color:blue;" onclick="deleteItemInfo(${item.thirdUnitId});">删除</a>
					</td>
				  </tr>
				</c:forEach>
			</c:if>
	...
	<div style="height:35px;">
		<div class="float_right padding5 paging">
			...
			<pg:pager maxPageItems="${pageSize }" url="fixItemAction!itemListInput.do" items="${pm.count}" export="page1=pageNumber">
			  	<pg:param name="jcsType" value="${jcsType}"/>
			  	<pg:param name="first" value="${first}"/>
			  	<pg:param name="second" value="${second}"/>
