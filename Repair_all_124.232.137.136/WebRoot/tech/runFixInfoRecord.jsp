...
<body>
	<form action="teach!runFixInfoRecord.do" method="post" id="subForm">
			   	...
			   	<c:forEach items="${DatePlanPriPm.datas}" var="datePlanPri" >
			   		<tr>
						...
						<td width="20%" align="center">
							<c:choose>
								<c:when test="${fn:contains(datePlanPri.fixFreque, 'Z') && datePlanPri.fixFreque != 'ZQZZ' && datePlanPri.fixFreque != 'QZ'}">
									<a href="query!getZxInfoByBZ.do?jcStype=${datePlanPri.jcType }&rjhmId=${datePlanPri.rjhmId }&xcxc=${datePlanPri.fixFreque }&workFlag=1" target="_blank">查看记录</a>
								</c:when>
								<c:otherwise>
									<a href="query!getInfoByBZ.do?jcStype=${datePlanPri.jcType }&rjhmId=${datePlanPri.rjhmId }&xcxc=${datePlanPri.fixFreque }&workFlag=1" target="_blank">查看记录</a>
								</c:otherwise>
							</c:choose>
						</td>
					</tr>
	...
	<div class="float_right padding5 paging">
		<div class="float_left padding_top4">
			<pg:pager maxPageItems="${pageSize }" url="teach!runFixInfoRecord.do" items="${DatePlanPriPm.count }" export="page1=pageNumber">
...
<script type="text/javascript">
	function fixView(rjhmId, fixFreque, jcStype){
		var diag = new top.Dialog();
		diag.Title = "记录查询窗口";
		if(fixFreque.indexOf('Z') >= 0 && fixFreque.indexOf('ZQZZ') <= 0 && fixFreque.indexOf('QZ') <= 0){
			diag.URL = "query!getZxInfoByBZ.do?jcStype="+jcStype+"&rjhmId="+rjhmId+"&xcxc="+fixFreque+"&workFlag=1";		
		}else{
			diag.URL = "query!getInfoByBZ.do?jcStype="+jcStype+"&rjhmId="+rjhmId+"&xcxc="+fixFreque+"&workFlag=1";
		}
		diag.Width=1200;
		diag.Height=520;
		diag.show();
