...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
		...
		function CreatePrintPage(){
			...
			LODOP.ADD_PRINT_TEXT(21,300,300,30,"${month }考勤月报\n");
		...
		function SaveAsFile(){ 
			...
			LODOP.SAVE_TO_FILE("${month }考勤月报.xls"); 
		};	
...
<div class="box2" panelTitle="考勤月汇总" roller="false">
	   <table>
		  <tr>
               ...
               <td><button onclick="query();" id="process"><span class="icon_find">查询</span></button></td>
               <td><button onclick="queryByXm();"><span class="icon_mark">详情展示</span></button></td>
		  </tr>
			  <tr >
			     <td colspan="4">
...
<div style="width:1750px;">
	<table  class="tableStyle" headFixMode="true"  useMultColor="true">
		<tr>
			<td style="width:60px;" align="center">班组</td>
			<td style="width:40px;" align="center">姓名</td>
			<td style="width:40px;" align="center">工号</td>
			<td style="width:40px;" align="center">应出天</td>
			<td style="width:40px;" align="center">实出天</td>
	<!-- 	<td style="width:40px;" align="center">应出时</td>   -->
	<!-- 	<td style="width:40px;" align="center">实出时</td>   -->
			<td style="width:40px;" align="center">补卡次</td>
			<td style="width:40px;" align="center">迟到次</td>
			<td style="width:40px;" align="center">早退次</td>
	<!-- 	<td style="width:40px;" align="center">旷工</td>  -->
			<td style="width:40px;" align="center">旷工次</td>
			<c:forEach begin="1" end="${maxMonthDay}" step="1" var="day">
				<td style="width:32px;" align="center">${day }</td>
			</c:forEach>
		</tr>
	<c:if test="${!empty monthCount}">
		<c:forEach var="monthCount" items="${monthCount}" >
	    <tr align="center">
	   	     <td>${monthCount.base.proteamName }</td>
	   	     <td>${monthCount.base.xm }</td>
	   	     <td>${monthCount.base.gonghao }</td>
	   	     <td>${monthCount.base.should_day }</td>
	   	     <td>${monthCount.base.real_day }</td>
	  <!--   <td>${monthCount.base.attendance }</td>   -->
	  <!-- 	 <td>${monthCount.base.real_attendance }</td>   --> 
	   	     <td>${monthCount.base.resign_count }</td>
	   	     <td>${monthCount.base.late_count }</td>
	   	     <td>${monthCount.base.goearly_count }</td>
	  <!--   <td>${monthCount.base.absenteeism }</td>   -->
	   	     <td>${monthCount.base.absenteeism_count }</td>
	   	     <c:forEach begin="1" end="${maxMonthDay}" step="1" var="key">
				<td onclick="resume('${monthCount.base.gonghao }','${key }');"><span style="color:red">${monthCount[key] }</span></td>
			</c:forEach>
	   </tr>
	</c:forEach>
	...
	<c:if test="${empty monthCount}">
	    <tr><td align="center" colspan="43">没有相应的数据信息!</td></tr>
	</c:if>
	</table>
	</div>
</div>
