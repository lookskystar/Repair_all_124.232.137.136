  ...
  <head>
	...
	<script type="text/javascript">
	   ...
	   function selectThis(obj){
           ...
           $.post("presetDivisionAction!getJCZxFixitemByYsId.do",{"ysId":ysId},function(data){
	   ...
	   function getpresetDivision(){
		   ...
		   if(bzid!=null && ''!=bzid && jcType!=null && ''!=jcType&&nodeId!=null && ''!=nodeId){
		   		window.location.href="presetDivisionAction!presetZxDivisionInput.do?jcType="+jcType+"&bzid="+bzid+"&nodeId="+nodeId+"&zxFlag=1";
		   }
	   }
	   ...
	   function savePresetItem(){
		   ...
		   if(presetId==""){
			  top.Dialog.alert("请选择预设分工大类!");
			   ...
			   $.post("presetDivisionAction!updateJCZxFixitemYsId.do",{"presetId":presetId,"ids":ids},function(data){
				  if(data=="success"){
	   ...
	   function updPreset(){
		      ...
		      if(presetId==""){
		    	 ...
		    	 top.Dialog.open({URL:"presetDivisionAction!updateZxPresetDivisionInput.do?presetId="+presetId+"&bzid="+bzid,Width:600,Height:450,Title:"修改预设分类"});
		      }
	   }
	   ...
	   function toAdd(){
	   		...
	   		var bzid = $("#bzid").val();
	   		var nodeId=$("#nodeId").val();
	   		if(bzid!=null && ''!=bzid &&nodeId!=null &&''!=nodeId){
	   			top.Dialog.open({URL:"presetDivisionAction!saveZxPresetDivisionInput.do?nodeId="+nodeId+"&jctype="+val+"&bzid="+bzid,Width:600,Height:450,Title:"添加预设分类"});
	   		}
	   }
	</script>
	<style type="text/css">
 ...
 <body>
 	<table width="100%" align="left">
 		...
 		<div class="box2"  panelTitle="预设分类管理" panelHeight="434" panelWidth="320" overflow="auto">
    	    ...
    	    <center>请选择项目所处节点名称:
 			<select style="width: 100px" onchange="getpresetDivision();" id="nodeId">
 				<option value="">请选择节点</option>
    	           <option value="100" <c:if test="${nodeId==100}">selected="selected"</c:if>>机车分解</option>
    	           <option value="101" <c:if test="${nodeId==101}">selected="selected"</c:if>>车上组装</option>
    	           <option value="102" <c:if test="${nodeId==102}">selected="selected"</c:if>>机车试验</option>
    	    </select>
    	    </center><br/>
 		   <center>请选择预分类的机车类型:
 					...
 					<script>
						...
						<c:forEach var="map" items="${itemMap}" varStatus="idx">
		 					...
		 					<c:forEach var="it" items="${map.value}">
		 					    <c:if test="${empty it.posiName}">d.add(${it.id},'parent_${idx.index*10+1}','<input type="checkbox" id="${it.id}" class="ch_${idx.index+1}" name="item"/>${it.itemName}','javascript:void(0)','${it.itemName}');</c:if>
		 					    <c:if test="${!empty it.posiName}">d.add(${it.id},'parent_${idx.index*10+1}','<input type="checkbox" id="${it.id}" class="ch_${idx.index+1}" name="item"/>${it.posiName}--${it.itemName}','javascript:void(0)','${it.posiName}--${it.itemName}');</c:if>
		 					</c:forEach>
		 				</c:forEach>
						document.write(d);
					</script> 
