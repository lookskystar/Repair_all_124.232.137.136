...
<button id="sign" onclick="javascript:void(0);" ><span class="icon_save">签名</span></button>
...
<table class="tableStyle" style="text-align: center;margin-top: 30px;" id="checkItemTab">
	...
	<c:if test="${xtype==1}">
		<c:if test="${empty itemList}"><tr><td colspan="9">没有数据记录!</td></tr></c:if>
		<c:forEach var="itm" items="${itemList}">
			<tr>
				<td>
					<c:choose>
						<c:when test="${itm.recStas>=4 && empty itm.accepter}"><input type="checkbox" value="${itm.preDictId}" name="check_item" id="check_item"/></c:when>
						<c:otherwise>&nbsp;</c:otherwise>
					</c:choose>
				</td>
	...
	<c:if test="${xtype==2}">
		...
		<c:forEach var="itm" items="${itemList}">
			<tr>
				<td>
					<c:choose>
						<c:when test="${itm.recStas>=2 && empty itm.accepter}"><input type="checkbox" value="${itm.jcRecId}" name="check_item" id="check_item"/></c:when>
						<c:otherwise>&nbsp;</c:otherwise>
					</c:choose>
				</td>
				<td>${itm.itemName}</td>
