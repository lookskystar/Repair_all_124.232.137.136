...
var getItem=function(teamId,id){
   var rjhmId="${rjhmId}";
   var xtype="${xtype}";
   $.post("<%=basePath%>signWorkAction!ajaxGetBzItem.do",{"teamId":teamId,"rjhmId":rjhmId,"xtype":xtype},function(data){
        ...
        if(datas.length>0){
            for(var i=0;i<datas.length;i++){
                ...
                if(parseData(datas[i].tech)==""&&datas[i].recstatus>=2){
                	str+="<td style='word-break:break-all;word-wrap: break-word;'><input type='checkbox' value="+datas[i].jcRecId+" name='check_item' class='ch_"+id+"'/></td>";
                }else{
                    str+="<td style='word-break:break-all;word-wrap: break-word;'></td>";
                }
                if(xtype==0){
	                str+="<td style='word-break:break-all;word-wrap: break-word;'>"+parseData(datas[i].secUnitName)+"&nbsp;"+parseData(datas[i].posiName)+"</td>";
                }
                str+="<td style='word-break:break-all;word-wrap: break-word;'>"+parseData(datas[i].itemName)+"</td>";
                if(parseData(datas[i].upPjNum) !=null ){
					str+="<td style='word-break:break-all;word-wrap: break-word;'>"+parseData(datas[i].fixSituation)+"<br/>"
					var nums = new Array();
					nums=parseData(datas[i].upPjNum).split(","); //字符分割   
					for(var j=0;j<nums.length;j++){
						str+="<a href='query!findPjRecordByPjNum.do?rjhmId=${rjhmId }&pjNum="+nums[j]+"' target='_blank' style='color:blue;'>"+nums[j]+"</a>";
					}
					str+="</td>";
                }else{
                     str+="<td style='word-break:break-all;word-wrap: break-word;'>"+parseData(datas[i].fixSituation)+"</td>";
                    }
                str+="<td style='word-break:break-all;word-wrap: break-word;'>"+parseEmpData(datas[i].fixEmp,1)+"&nbsp;"+parseData(datas[i].empAfformTime)+"</td>";
                str+="<td style='word-break:break-all;word-wrap: break-word;' >"+parseEmpData(datas[i].lead,0)+"&nbsp;"+parseData(datas[i].ldAffirmTime)+"</td>";
                if(datas[i].itemCtrlQI==1){
                   str+="<td style='word-break:break-all;word-wrap: break-word;'>"+parseEmpData(datas[i].qi,0)+"&nbsp;"+parseData(datas[i].qiAffiTime)+"</td>";
                }else{
                   str+="<td style='word-break:break-all;word-wrap: break-word;' >"+parseData(datas[i].qi)+"&nbsp;"+parseData(datas[i].qiAffiTime)+"</td>";
                }
                if(datas[i].itemCtrlTech==1){
                    str+="<td style='word-break:break-all;word-wrap: break-word;'>"+parseEmpData(datas[i].tech,0)+"&nbsp;"+parseData(datas[i].techAffiTime)+"</td>";
                }else{
                    str+="<td style='word-break:break-all;word-wrap: break-word;'>"+parseData(datas[i].tech,0)+"&nbsp;"+parseData(datas[i].techAffiTime)+"</td>";
                }
                str+="<td style='word-break:break-all;word-wrap: break-word;'>"+parseData(datas[i].commitLead)+"&nbsp;"+parseData(datas[i].comLdAffiTime)+"</td>";
                str+="<td style='word-break:break-all;word-wrap: break-word;'>"+parseData(datas[i].accepter)+"&nbsp;"+parseData(datas[i].acceAffiTime)+"</td>";
                str+="</tr>";
            }
        }else{
...
<button id="allsign" ><span class="icon_save">全签</span></button>
...
<table class="tableStyle" style="text-align: center;margin-top: 30px;" id="checkItemTab">
	...
	<tr>
		<th width="40px">选择</th>
		<c:if test="${xtype==0}">
			<th width="12%">部件/部位</th>
		</c:if>
		<th width="29%">检修内容/项目名称${itemList}</th>
		<th width="5%">检修情况</th>
		<th width="9%">检修人</th>
		<th width="9%">工长</th>
		<th width="9%">质检员</th>
	...
	<c:if test="${xtype==0 || xtype == 2}">
		<c:forEach var="itm" items="${itemList}">
			<tr>
				<td>
					<c:choose>
						<c:when test="${itm.recStas>=2 && empty itm.tech}">
				...
				<td>
				   ...
				   <c:if test="${!empty itm.upPjNum}">${itm.fixSituation}${itm.fixitem.unit}<br/>
				       <a href="query!findPjRecordByPjNum.do?rjhmId=${itm.datePlanPri.rjhmId}&pjNum=${itm.upPjNum }" target="_blank" style="color:blue;">${itm.upPjNum }</a>
				   </c:if>
				 </td>
				<td>
					<c:choose>
	...
	<c:if test="${xtype==1}">
		<c:forEach var="itm" items="${itemList}">
			<tr>
				<td>
					<c:choose>
						<c:when test="${itm.recStas>=4 &&(empty itm.technician)}"><input type="checkbox" value="${itm.preDictId}" name="check_item" id="check_item"/></c:when>
						<c:otherwise>&nbsp;</c:otherwise>
					</c:choose>
				</td>
	...
	<c:if test="${xtype==2}">
		<c:forEach var="itm" items="${itemList}">
			<tr>
				<td>
					<c:choose>
						<c:when test="${itm.recStas>=2 && empty itm.tech}"><input type="checkbox" value="${itm.jcRecId}" name="check_item" id="check_item"/></c:when>
						<c:otherwise>&nbsp;</c:otherwise>
					</c:choose>
				</td>
				<td>${itm.itemName}</td>
...
<table class="treeTable" style="text-align: center;margin-top: 30px;" initState="collapsed" id="freeItemTab">
	<%--
   <tr style="background-color: lightblue;"><td colspan="11" align="left" style="font-weight: bolder;"><a id="aa">自选签认项目</a>&nbsp;&nbsp;<font color="red">(*自选项目不能全签)</font><button anchor="bz" id="nbz">签认项目</button></td></tr>
	 --%>
   <tr>
		<th width="40">选择</th>
		<c:if test="${xtype==0}">
