			...
			<table width="100%" class="tableStyle">
				...
				<c:if test="${!empty datePlanPris}">
					<c:forEach items="${datePlanPris}" var="entry">
						<tr align="center">
							<td class="ver01">
								${entry.jcType}-${entry.jcnum}-${entry.fixFreque}
							</td>
							<td class="ver01">
								${entry.kcsj}
							</td>
							<td class="ver01">
								<a href="reportWorkManage!reportWorkSignItem.action?id=${entry.rjhmId}&role=${role}&flag=${flag}" class="icon_view hand" target="frmright">签认项目</a>
							</td>
						</tr>
					</c:forEach>
				</c:if>
