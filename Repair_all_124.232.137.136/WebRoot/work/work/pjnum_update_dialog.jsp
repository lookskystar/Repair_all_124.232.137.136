...
<script type="text/javascript">
	   ...
	   if(pjNums.length!=len){
		   top.Dialog.alert("编号不能为空!");
	   	   ...
	   	   if(flag){
			   ...
			   $.post("<%=basePath%>workAction!savePjNum.do",{"zxRecId":zxRecId,"pjNum":pjNum,"pjNumsOld":pjNumsOld},function(data){
  ...
  function choicePart(obj){
		...
		diag.Title = "选择配件";
		diag.Width = 700;
		diag.URL = '<%=basePath%>workAction!choiceDyPjInfo.do?stdPjId=${stdPjId}&planId=${planId}&type=update';
		...
		<c:when	test="${empty amount}">
			<tr>
				...
				<td>
				  ...
				  <input type="button" value="选择" onclick="choicePart(this);" readonly="readonly"/>&nbsp;&nbsp;
