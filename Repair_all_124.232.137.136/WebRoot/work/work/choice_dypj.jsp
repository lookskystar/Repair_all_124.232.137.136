...
<script type="text/javascript">
   ...
   function getFirstUnit(){
    ...
    function addPjDyInfo(){
    	top.Dialog.open({URL:"<%=basePath%>pjManageAction!addPjDyInfoInput.do",Width:650,Height:280,Title:"添加静态配件信息"});
    }
     //修改动态配件信息
    ...
    function delPjDyInfo(pjId){
    	...
    	top.Dialog.confirm("确认要删除吗?",function(){
    		...
    		if(pjIdArry.length==0){
    			...
    			window.location.href="<%=basePath%>pjManageAction!delPjDyInfo.do?pjIds="+pjIds;
    		}
    	});
    }
</script>
...
<body>
<form id="form" action="workAction!choiceDyPjInfo.do?stdpjid=${stdpjid }&planId=${planId }" method="post">
  ...
  配件编号:<input type="text" style="width: 200px" name="pjNum" value="${pjNum }"/>&nbsp;&nbsp;&nbsp;
      <!-- 2015-5-21，黄亮，改， 去掉只读属性readonly="readonly"-->
	   ...
	   <table width="100%" class="tableStyle" useMultColor="true"  useCheckBox="true">
			<tr>
			    ...
			    <th class="ver01" width="5%" align="center">备注</th>
			    <th class="ver01" width="8%" align="center">操作</th>
			</tr>
			<c:if test="${!empty pm.datas}">
				<c:forEach items="${pm.datas}" var="pjd">
				   <tr>
				   <td class="ver01" align="center"><input type="checkbox" id="${pjd.pjdid}" name="pjIds" value="${pjd.pjNum}"/></td>
