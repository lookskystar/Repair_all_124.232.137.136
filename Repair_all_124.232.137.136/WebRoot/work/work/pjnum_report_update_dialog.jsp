...
<!-- 2015-5-25,黄亮，添加，新页面：pjnum_report_update_dialog.jsp。目的：报活项目中，处理情况列，点击配件编码，修改报活配件-->
<%
...
function submitFormNew(){
	   ...
	   if(pjNums.length!=len){
		   top.Dialog.alert("编号不能为空!");
	   	   ...
	   	   //2015-5-29，黄亮，bug：隔行添加同样配件编号，不会提示：所填编号不能相同，后面解决
	   	   ...
	   	   if(flag){
	   	       ...
	   	       //2015-5-21，黄亮，改，传入pjdid：pjNum字符串形式和predictId报活记录id
	   	       //2015-5-21，黄亮，添加，如果type==2，则刷新frames[2]页面（frames[0]：主页面，frames[1]：左页面，frames[2]：右边页面）
	   	       $.post("<%=basePath%>workAction!updatePJDynamicInfosByPjNumAndJtPreDict.do?pjNums="+pjNums,{"planId":planId,"predictId":predictId,"pjNumsOld":pjNumsOld},function(data){
