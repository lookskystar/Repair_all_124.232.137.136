...
<form id="subForm" action="workAction!finishItemSignWoker.action?" method="post">
...
<table class="treeTable" style="text-align: center;margin-top: 30px;" initState="collapsed" id="freeItemTab">
    ...
    <c:forEach var="itemListMap" items="${itemListMap}" varStatus="s">
	    <tr id="node-${s.index+1 }">
			...
			<td colspan="4" align="left"><span class="folder">${itemListMap.key }</span></td>
		</tr>
		<c:forEach var="rec" items="${itemListMap.value}">
			<tr class="child-of-node-${s.index+1}" onclick="select(${rec.recId});">
			    <td>
					<input type="checkbox" value="${rec.recId}" class="ch_${s.index+1}" name="jcsy_item" id="checked-${rec.recId}"/>
				</td> 
				<td>${rec.classify}</td>
				<td>${rec.itemName}</td>
				<td align="left">
					<input type="hidden" id="${rec.recId}-max" value="${rec.item.max}" />
					<input type="hidden" id="${rec.recId}-min" value="${rec.item.min}"/>
					<input type="hidden" id="${rec.recId}-check" value="${rec.item.isCheck}"/>
					<input type="text" id="${rec.recId}" value="${rec.fixSituation}"/>${rec.unit}
				</td>
				<td>${rec.fixemp}&nbsp;${rec.fixEmpTime}</td>
...
<script type="text/javascript">
	...
	function select(runId){
		$("#checked-"+ runId +"").attr("checked","checked");
	}
	...
	function ratify(){
		...
		if(flagSelect){
			...
			$("input[name='jcsy_item']:checked").each(function(){
			    ...
			    var isChecked = $("#"+ idValue +"-check").val();
			    if(isChecked == 1){
