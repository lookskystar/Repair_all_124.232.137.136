...
<body><br/>
    <form action="pjManageAction!addPjStaticInfo.do" method="post" target="frmright" onsubmit="return validate();"> 
        <input type="hidden" name="pjStatic.jcType" id="jcType"/>
		<table class="tableStyle" transMode="true"><!-- transMode="true" -->
		   <tr>
				<td>配件名称: </td>
				<td colspan="3">
				   <input class="easyui-validatebox" type="text" name="pjStatic.pjName" style="width: 300px;" id="pjName"></input>
				</td>
			</tr>
		    <tr>
				<td>最小库存量: </td>
				<td>
				   <input class="easyui-validatebox" type="text" name="pjStatic.lowestStore" onkeyup="this.value=this.value.replace(/\D/g,'')"></input>
				</td>
			
				<td>最大库存量: </td>
				<td>
				   <input class="easyui-validatebox" type="text" name="pjStatic.mostStore" onkeyup="this.value=this.value.replace(/\D/g,'')"></input>
				</td>
			</tr>
		   	<tr>
				<td>适用车型: </td>
				<td colspan="3">
				   <c:forEach var="jc" items="${jcsTypes}">
				     <input type="checkbox" value="${jc.jcStypeValue }" name="jcTypes"/>${jc.jcStypeValue }
				   </c:forEach>
				</td>
			</tr>
			<tr>
